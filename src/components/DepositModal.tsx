...
export default function DepositModal({
  ...
  // Modal açıldığında mevcut borç tutarını varsayılan (default) değer olarak getir
  useEffect(() => {
    if (isOpen) {
  ...
  const handlePayDebt = async (e: React.FormEvent) => {
    ...
    if (numAmount > currentDebtTL) {
      setError(`Ödenecek tutar mevcut borcunuzdan (₺${currentDebtTL}) fazla olamaz.`);
      return;
    }
        ...
        <form onSubmit={handlePayDebt} className="mt-5 space-y-4">
          ...
          <div className="p-4 bg-amber-500/10 border border-amber-500/30 rounded-2xl flex items-center justify-between text-amber-200">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <span className="text-xs text-amber-300/80 font-medium">Güncel Toplam Borcunuz</span>
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1.5">
              İsterseniz tutarı değiştirerek kısmi ödeme gerçekleştirebilirsiniz.
            </p>
          </div>
          ...
          <div className="pt-2 flex items-center space-x-3">
            <button
              ...
              onClick={onClose}
              disabled={loading}
              className="w-1/3 py-3.5 px-4 rounded-2xl border border-slate-800 bg-slate-800/50 hover:bg-slate-800 text-slate-300 font-medium transition-colors disabled:opacity-50"
            >
              Vazgeç
            </button>
            <button
              type="submit"
              disabled={loading}
              className="w-2/3 py-3.5 px-4 rounded-2xl bg-gradient-to-r from-amber-500 to-orange-600 hover:from-amber-600 hover:to-orange-700 text-slate-950 font-bold shadow-lg shadow-amber-950/50 transition-all disabled:opacity-50 flex items-center justify-center space-x-2"
            >
              {loading ? (
                <>
